  ...
  while( 1 )
    {
      ...
      if( strcmp_PF(cmd, PSTR("a:"))==0 || strcmp_PF(cmd, PSTR("b:"))==0 )
        ...
        {
          Serial.println("Exiting ArduDOS");
          return;
        }
#if 0
      ...
      else if( strncmp_P(cmd, PSTR("rename "), 7)==0 )
        {
          ArduinoFDC.motorOn();
...
bool xmodemHandlerReceiveMon(unsigned long no, char* data, int size)
{
  ...
  if( xmodem_data_ptr>=512 )
    {
    ...
    #if 0
      // Provide sector skew based on performance
      const uint8_t skewMap2[]  = {1, 3, 5, 7, 9, 11, 13, 15, 17, 2, 4, 6, 8, 10, 12, 14, 16, 18};
      const uint8_t skewMap3[]  = {1, 4, 7, 10, 13, 46, 2, 5, 8, 11, 14, 17, 3, 6, 9, 12, 15, 18};
      ...
      sector = skewMap12[sector-1];
    #endif

      performSectorSkew(&sector, 2, numsec);

      byte r = S_NOHEADER, retry = 5;
      while( retry>0 && r!=S_OK ) { r = ArduinoFDC.writeSector(track, head, sector, databuffer, xmodem_verify); retry--; }
      if( r!=S_OK ) { xmodem_status_mon = r; return false; }
      ...
      if( cmd=='r' && n>=3 )
        {
          ...
          if( head>=0 && head<2 && track>=0 && track<ArduinoFDC.numTracks() && sector>=1 && sector<=ArduinoFDC.numSectors() )
            {
              Serial.print(F("Rding track ")); Serial.print(track); 
                    ...
                    while( true )
                      {
                        ...
                        if( status==S_OK )
                          ...
                          Serial.println(F(" => CRC error, retry"));
                        else
                          {
                            Serial.print(F(" => "));
                            print_error(status);
      ...
      else if( cmd=='w' && n>=3 )
        {
          ...
          if( head>=0 && head<2 && track>=0 && track<ArduinoFDC.numTracks() && sector>=1 && sector<=ArduinoFDC.numSectors() )
            {
              Serial.print(F("Write/verify track ")); Serial.print(track); 
              Serial.print(F(" sector ")); Serial.print(sector);
              Serial.print(F(" side ")); Serial.println(head);
              Serial.flush();
            ...
            Serial.println(F("Invalid sector spec"));
        }
      else if( cmd=='w' && n>=1 )
        {
          bool verify = n>1 && a2>0;
          char c;
          Serial.print(F("Write buff to all sectors"));
          Serial.write('A' + ArduinoFDC.selectedDrive());
                ...
                for(head=0; head<2; head++)
                  {
                    ...
                    for(byte i=0; i<ArduinoFDC.numSectors(); i++)
                      {
                        Serial.print(verify ? F("Write/verify track ") : F("Writing track ")); Serial.print(track);
                        Serial.print(F(" sector ")); Serial.print(sector);
                        Serial.print(F(" side ")); Serial.print(head);
                        Serial.flush();
        ...
        return;
#endif
#if 1 || (!defined(USE_ARDUDOS) || !defined(USE_MONITOR) || !defined(USE_XMODEM) || defined(__AVR_ATmega2560__))
      // must save flash space if all three of ARDUDOS/MONITR/XMODEM are enabled on UNO
      else if( cmd=='h' || cmd=='?' )
        {
